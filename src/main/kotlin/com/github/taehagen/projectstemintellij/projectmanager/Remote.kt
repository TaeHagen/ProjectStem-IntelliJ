...
object Remote {
    ...
    fun getCourses(user: User, courses: ArrayList<Course>): Boolean {
        val req = Request.Builder()
            .url("https://courses.projectstem.org/api/v1/dashboard/dashboard_cards")
            .header("cookie", "_normandy_session=${user.token}")
            .header("accept", "application/json")
            .build();
        val res = client.newCall(req).execute()
        if (res.code != 200) {
        ...
        (0 until json.length()).forEach {
            ...
            courses.add(Course(user, course.getInt("id"), course.getString("shortName")))
        }
        return true
    }

    fun getModules(user: User, course: Course, modules: ArrayList<Module>): Boolean {
        val req = Request.Builder()
    ...
    fun getItems(user: User, module: Module, items: ArrayList<Item>): Boolean {
        var page = 1
        while (true) {
            val req = Request.Builder()
